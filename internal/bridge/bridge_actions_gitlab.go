...
func bridgeActionAwaitGLRelease(ctx context.Context, inputs map[string]interface{}, credStore *CredentialStore, teamID string) (*BridgeActionResult, error) {
	...
	// URL-encode the project path and tag name for API safety
	encodedProject := strings.ReplaceAll(project, "/", "%2F")
	...
	for time.Now().Before(deadline) {
		...
		if err := json.Unmarshal(respBody, &release); err != nil {
			return &BridgeActionResult{
				Status: "failed",
				Error:  fmt.Sprintf("failed to parse GitLab release response: %v", err),
			}, nil
		}

		log.Printf("bridge-action await-gl-release: release %s found for %s at %s", tag, project, release.WebURL)
		return &BridgeActionResult{
			Status: "succeeded",
			Outputs: map[string]interface{}{
				"release_url": release.WebURL,
			},
		}, nil
	}
...
func bridgeActionCreateGLIssue(ctx context.Context, inputs map[string]interface{}, credStore *CredentialStore, teamID string) (*BridgeActionResult, error) {
	...
	if project == "" || title == "" {
		return &BridgeActionResult{
			Status: "failed",
			Error:  "missing required inputs: project, title",
		}, nil
	}
	...
	if err != nil {
		return &BridgeActionResult{
			Status: "failed",
			Error:  fmt.Sprintf("GitLab API error creating issue: %v", err),
		}, nil
	}
	...
	if err := json.Unmarshal(respBody, &issueResp); err != nil {
		return &BridgeActionResult{
			Status: "failed",
			Error:  fmt.Sprintf("failed to parse GitLab issue response: %v", err),
		}, nil
	}
...
// bridgeActionCreateMRs creates merge requests across multiple GitLab projects.
func bridgeActionCreateMRs(ctx context.Context, inputs map[string]interface{}, credStore *CredentialStore, teamID string) (*BridgeActionResult, error) {
	...
	if len(projects) == 0 {
		return &BridgeActionResult{Status: "failed", Error: "missing required input: projects (array of GitLab project paths)"}, nil
	}
			...
			if strings.Contains(err.Error(), "409") || strings.Contains(err.Error(), "already exists") {
				...
				if findErr == nil {
					...
					if json.Unmarshal(existingBody, &existing) == nil && len(existing) > 0 {
						log.Printf("bridge-action create-mrs: MR already exists for branch %s in %s: !%d", sourceBranch, project, existing[0].IID)
						mrIIDs = append(mrIIDs, existing[0].IID)
						mrURLs = append(mrURLs, existing[0].WebURL)
