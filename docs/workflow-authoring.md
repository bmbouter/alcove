...
The step status reflects the CI outcome: when CI passes, the step succeeds 
(status `completed`); when CI fails, the step fails (status `failed`). The 
CI outcome is also available in the `status` output: `passed` if all checks 
succeed or are skipped, `failed` if any check fails. When CI fails, 
`failure_logs` contains the last 3000 characters of each failed check's log 
output.

**Note:** `await-ci.Failed` means "CI failed" (not "the API call failed"). 
This allows steps like `ci-fix` to dispatch when CI actually fails.
...
1. `await-ci -> ci-fix -> await-ci` -- CI failure handling (fires when CI fails)
